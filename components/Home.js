...
function Home() {
  ...
  useEffect(() => {
    fetch('https://my-moviz-backend-omega.vercel.app/movies')
      .then(res => res.json())
      .then(data => {
  ...
  */
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={truncateText(data.overview, 250)} poster={'https://image.tmdb.org/t/p/w500' + data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
